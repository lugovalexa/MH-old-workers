...
data$work_horizon_change_bin2 <- ifelse(data$work_horizon_change >= 2, 1, 0)
...
data2015 <- data %>%
  filter(time == 1) %>%
  dplyr::select(mergeid, country, work_horizon_change)
data2015$cell1 <- paste(data2015$country, data2015$work_horizon_change, sep = "_")
data <- data %>%
...
```{r}
fit1 <- plm(eurod ~ did,
                       data=subset(data,gender==0), weights=my_wgt, index=c("mergeid","year"),method="within",effect="twoways")
summary(fit1, robust = "vcovHC")
```
```{r}
fit <- feols(eurod ~ did | cell1, data = subset(data,gender==0))

summary(fit, robust = "gee")
```

```{r}
fit <- fepois(eurod ~ time:work_horizon_change_bin1 | cell1, data = subset(data,gender==0))
...
```{r}
fit1 <- glm(eurod ~ time + treated + did,
                       data=subset(data,gender==0), weights=my_wgt, family="quasipoisson")
summary(fit1, robust = "vcovHC")
```


```{r}
fit1 <- lm(eurod_log ~ did + time + treated,
...
```{r}
fit2 <- lm(eurod_log  ~ did + treated + time + as.factor(cell),
                       data=subset(data,gender==0),  weights=my_wgt)
summary(fit2, robust = "vcovHC")
```


```{r}
clustered_se <- vcovHC(fit2, type = "HC1", cluster = "cell") # Cluster standard errors at the country level
robust_se <- sqrt(diag(clustered_se)) # Obtain robust standard errors
...
```{r}
fit3 <- lm(eurod_log ~ did + time + treated + partnerinhh + nb_children + nb_grandchildren + yrseducation + sphus + chronic
               + life_insurance + investment + thinc_log + work_horizon + jqi_sum + as.factor(cell) + as.factor(industry),
                       data=subset(data,gender==0), weights=my_wgt)
summary(fit3, robust = "vcovHC")
```

```{r}
clustered_se <- vcovHC(fit3, type = "HC1", cluster = "cell") # Cluster standard errors at the country level
robust_se <- sqrt(diag(clustered_se)) # Obtain robust standard errors
...
```{r}
fit4 <- glm(eurod ~ did + time + treated + partnerinhh + nb_children + nb_grandchildren + yrseducation + sphus + chronic
               + life_insurance + investment + thinc_log + work_horizon + jqi_physical_environment + jqi_social_environment
               + jqi_intensity + jqi_prospects + jqi_working_time_quality + as.factor(cell) + as.factor(industry),
                       data=subset(data,gender==0), weights=my_wgt, family="quasipoisson")
...
```{r}
bptest(fit2) #small p-value - not normal - heteroscedasticity present
```
...
full_formula <- reformulate(c("did", "time", "partnerinhh", "nb_children", "nb_grandchildren", "yrseducation", "sphus", "chronic",
               "life_insurance", "investment", "thinc_log", "work_horizon", "jqi_physical_environment", "jqi_social_environment",
               "jqi_intensity", "jqi_prospects", "jqi_working_time_quality", "as.factor(cell)", "as.factor(industry)"),response="eurod_log")
noind_formula <- reformulate(c("did", "time", "partnerinhh", "nb_children", "nb_grandchildren", "yrseducation", "sphus", "chronic",
               "life_insurance", "investment", "thinc_log", "work_horizon", "jqi_sum", "as.factor(cell)", "as.factor(industry)"),response="eurod_log")
```

```{r}
full_formula <- reformulate(c("did", "time", "treated", "as.factor(cell)"),response="eurod_log")
noind_formula <- reformulate(c("did", "time", "treated", "as.factor(cell)"),response="eurod_log")
```


```{r}
# DID full dataset panel fixed effects
did_full = lm(eurod_log ~ did + time + gender + partnerinhh + nb_children + nb_grandchildren + yrseducation + sphus + chronic
               + life_insurance + investment + thinc_log + work_horizon + jqi_physical_environment + jqi_social_environment
               + jqi_intensity + jqi_prospects + jqi_working_time_quality + as.factor(cell) + as.factor(industry),
               data = data, weights = my_wgt)
...
```{r}
interaction_formula <- paste(interaction_terms_colnames, collapse = " + ")
full_formula <- reformulate(c("did", "time", "treated", "as.factor(cell)"),response="eurod")
noind_formula <- reformulate(c("did", "time", "treated", "as.factor(cell)"),response="eurod")
```
...
```{r}
crosstable(subset(data, time == 1), c(eurod, eurodcat), by=treated, test=TRUE) %>%
