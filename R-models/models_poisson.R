...
# Basic model
fit1 <- glm(eurod ~ post*treated0, data=data, weights=my_wgt, family="quasipoisson")
summary(fit1, robust = "vcovHC")

# Model with cell fixed effects
fit2 <- glm(eurod ~ post*treated0 + as.factor(cell1), data=data, weights=my_wgt, family="quasipoisson")
summary(fit2, robust = "vcovHC")

# Full model without all indexes
fit3 <- glm(eurod ~ post*treated0 + partnerinhh + nb_children + nb_grandchildren + yrseducation + sphus + chronic
...
summary(fit3, robust = "vcovHC")$coefficients['treated0',]

# Full model
fit4 <- glm(eurod ~ post*treated0 + partnerinhh + nb_children + nb_grandchildren + yrseducation + sphus + chronic
           ...
           data=datam, weights=my_wgt, family="quasipoisson")
summary(fit4, robust = "vcovHC")$coefficients['treated0',]
...
full_formula <- reformulate(c("post*treated0", "partnerinhh", "nb_children", "nb_grandchildren", "yrseducation", "sphus", "chronic",
                              "life_insurance", "investment", "thinc_log", "work_horizon", "jqi_physical_environment", "jqi_social_environment",
                              "jqi_intensity", "jqi_prospects", "jqi_working_time_quality", "as.factor(cell2)", "as.factor(industry)"),response="eurod")
noind_formula <- reformulate(c("post*treated0", "partnerinhh", "nb_children", "nb_grandchildren", "yrseducation", "sphus", "chronic",
                               "life_insurance", "investment", "thinc_log", "work_horizon", "jqi_sum", "as.factor(cell2)", "as.factor(industry)"),response="eurod")
